import { parse, parser, ParseError, Parser } from '.';

export const identity = <T, U>(value: U) =>
  parser<T, U>((input: T) => [value, input]);

export const bind = <T, U, V>(
  p: Parser<T, U>,
  m: (x: U) => Parser<T, V>
): Parser<T, V> =>
  function*() {
...
export const union = <T, P extends NonEmptyArray<Parser<T, unknown>>>(
  ...parsers: P
) =>
  parser((input: T) => {
...
export const list = <T, U>(p: Parser<T, U>): Parser<T, U[]> =>
  union(list1(p), identity<T, U[]>([]));

export const list1 = <T, U>(p: Parser<T, U>): Parser<T, U[]> =>
  function*() {
    const first = yield* p();
...
export const sepby = <T, U, V>(
  p: Parser<T, U>,
  s: Parser<T, V>
): Parser<T, U[]> => union(sepby1(p, s), identity<T, U[]>([]));

export const sepby1 = <T, U, V>(
  p: Parser<T, U>,
  s: Parser<T, V>
): Parser<T, U[]> =>
  function*() {
...
export const repeat = <T, U>(times: number, p: Parser<T, U>): Parser<T, U[]> =>
  function*() {
    const results = [];
